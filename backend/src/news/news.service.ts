...
export class NewsService {
  ...
  private async bootstrapFromLegacyFile() {
    ...
    const count = await this.prisma.newsPoster.count();
    ...
    try {
      ...
      await this.prisma.newsPoster.createMany({
        data: legacyItems
  ...
  async getCurrent() {
    ...
    return this.prisma.newsPoster.findFirst({
      where: { isActive: true },
  ...
  async findAll() {
    await this.bootstrapFromLegacyFile();
    return this.prisma.newsPoster.findMany({ orderBy: { createdAt: 'desc' } });
  }

  async create(dto: CreateNewsDto) {
    await this.bootstrapFromLegacyFile();
    return this.prisma.newsPoster.create({
      data: {
  ...
  async update(id: string, dto: UpdateNewsDto) {
    await this.bootstrapFromLegacyFile();
    const existing = await this.prisma.newsPoster.findUnique({ where: { id } });
    if (!existing) throw new NotFoundException('News poster not found');
    return this.prisma.newsPoster.update({
      where: { id },
      data: {
  ...
  async remove(id: string) {
    ...
    const existing = await this.prisma.newsPoster.findUnique({ where: { id } });
    if (!existing) throw new NotFoundException('News poster not found');
    return this.prisma.newsPoster.delete({ where: { id } });
  }
}
