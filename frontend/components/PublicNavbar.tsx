  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-cyan-300/20 bg-[#0a1330]/80 backdrop-blur-xl">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4">
          <Link href="/dashboard" className="group relative flex items-center gap-3 text-xl font-black tracking-wide text-white">
            ...
            {logoUrl ? (
              <img src={logoUrl} alt="CashlySweeps logo" className="h-10 w-10 rounded-xl object-cover ring-1 ring-cyan-300/50 shadow-[0_0_24px_rgba(34,211,238,0.35)] transition-transform duration-300 group-hover:scale-105" />
            ) : (
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 to-cyan-400 text-sm font-black shadow-[0_0_24px_rgba(34,211,238,0.35)]">CS</span>
            )}
              ...
              return (
                <Link
                  ...
                  className={`group relative overflow-hidden rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 ${
                    active ? "bg-gradient-to-r from-fuchsia-500/85 via-violet-500/80 to-cyan-400/85 text-white shadow-[0_0_28px_rgba(34,211,238,0.45)]" : "text-cyan-100/80 hover:-translate-y-0.5 hover:scale-[1.03] hover:text-white"
                  }`}
                >
                  {!active && <span className="absolute inset-0 -z-10 bg-gradient-to-r from-fuchsia-500/0 via-fuchsia-500/25 to-cyan-400/0 opacity-0 transition duration-300 group-hover:opacity-100" />}
                  {!active && <span className="absolute bottom-0 left-1/2 h-px w-0 -translate-x-1/2 bg-gradient-to-r from-fuchsia-400 to-cyan-300 transition-all duration-300 group-hover:w-3/4" />}
                  {item.label}
                </Link>
              );
            })}
          ...
          <div className="hidden items-center gap-2 lg:flex">
            {contacts?.facebook && (
              <a href={contacts.facebook} target="_blank" rel="noreferrer" aria-label="Facebook" title="Facebook" className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-fuchsia-300/40 bg-fuchsia-500/15 text-fuchsia-100 shadow-[0_0_18px_rgba(217,70,239,0.35)] transition hover:-translate-y-0.5 hover:bg-fuchsia-500/25">
            ...
            {contacts?.whatsapp && (
              <a href={whatsappLink} target="_blank" rel="noreferrer" aria-label="WhatsApp" title="WhatsApp" className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-emerald-300/40 bg-emerald-500/15 text-emerald-100 shadow-[0_0_18px_rgba(16,185,129,0.35)] transition hover:-translate-y-0.5 hover:bg-emerald-500/25">
          ...
          <button type="button" onClick={() => setMobileNavOpen(true)} aria-label="Open menu" className="flex h-10 w-10 items-center justify-center rounded-xl border border-fuchsia-300/40 bg-fuchsia-500/15 text-fuchsia-100 shadow-[0_0_18px_rgba(217,70,239,0.35)] transition hover:bg-fuchsia-500/25 lg:hidden">
      ...
      <div role="dialog" aria-modal="true" aria-label="Navigation menu" className={`fixed inset-0 z-[60] lg:hidden ${mobileNavOpen ? "visible" : "invisible"}`}>
        ...
        <div className={`absolute right-0 top-0 flex h-full w-[min(320px,85vw)] flex-col border-l border-cyan-300/30 bg-[#0a1330]/98 shadow-[0_0_60px_rgba(217,70,239,0.25)] transition-transform duration-300 ease-out ${mobileNavOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="flex items-center justify-between border-b border-cyan-300/20 px-4 py-4">
          ...
          <nav className="flex flex-1 flex-col gap-1 p-4">
            {NAV_ITEMS.map((item) => {
              const active = item.match(pathname);
              return (
                <Link key={item.label} href={item.href} onClick={() => setMobileNavOpen(false)} className={`rounded-xl px-4 py-3 text-base font-medium transition ${active ? "bg-gradient-to-r from-fuchsia-500/40 to-cyan-400/40 text-white" : "text-cyan-100/90 hover:bg-cyan-500/15"}`}>
                  {item.label}
                </Link>
              );
            })}
          ...
          <div className="border-t border-cyan-300/20 p-4">
            <div className="flex justify-center gap-3">
              {contacts?.facebook && (
                <a href={contacts.facebook} target="_blank" rel="noreferrer" aria-label="Facebook" className="flex h-10 w-10 items-center justify-center rounded-full border border-fuchsia-300/40 bg-fuchsia-500/15 text-fuchsia-100 shadow-[0_0_18px_rgba(217,70,239,0.35)]">
              ...
              {contacts?.whatsapp && (
                <a href={whatsappLink} target="_blank" rel="noreferrer" aria-label="WhatsApp" className="flex h-10 w-10 items-center justify-center rounded-full border border-emerald-300/40 bg-emerald-500/15 text-emerald-100 shadow-[0_0_18px_rgba(16,185,129,0.35)]">
