    ...
    <div className="space-y-8 text-white">
      ...
      <section className="auth-card space-y-4 rounded-2xl border border-zinc-700/40 bg-zinc-900/60 p-6">
        ...
        <div className="grid gap-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-zinc-300">Upload blog image (optional)</label>
            <input type="file" accept="image/*" onChange={(e) => uploadImage(e.target.files?.[0] ?? null)} className="rounded-lg border border-zinc-600 bg-zinc-800 px-3 py-2 text-sm text-zinc-200" />
            {uploading && <p className="mt-1 text-xs text-zinc-400">Uploading...</p>}
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-zinc-300">Full blog content</label>
            <textarea value={form.content} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))} rows={5} className="w-full rounded-lg border border-zinc-600 bg-zinc-800 px-3 py-2 text-zinc-100" />
          </div>
          <Button onClick={addBlog} loading={saving} disabled={!form.title.trim()}>Add Blog</Button>
        </div>

        <div className="mt-6 space-y-3">
          <h3 className="font-semibold">Existing blogs</h3>
          {blogs.length === 0 ? (
